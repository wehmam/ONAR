                        ...
                        <div class="col-md-3">
                            <div class="col-md-12">
                                <div class="checklist categories">
                                    ...
                                    <ul>
                                        <li><a type="submit" href="{{ request()->url() . (empty($queryParams) ? "?type=all" : "?" . http_build_query(array_merge($queryParams, ['type' => 'all'])) ) }}"><h5><i class="{{ request()->get("type") == "all" ? "fa fa-check" : "" }} fa-xs"></i> All</h5></a></li>
                                        <li><a type="submit" href="{{ request()->url() . (empty($queryParams) ? "?type=online" :  "?" . http_build_query(array_merge($queryParams, ['type' => 'online'])) ) }}"><h5><i class="{{ request()->get("type") == "online" ? "fa fa-check" : "" }} fa-xs"></i> Online</h5></a></li>
                                        <li><a type="submit" href="{{ request()->url() . (empty($queryParams) ? "?type=offline" :  "?" . http_build_query(array_merge($queryParams, ['type' => 'offline'])) ) }}"><h5><i class="{{ request()->get("type") == "offline" ? "fa fa-check" : "" }} fa-xs"></i> Offline</h5></a></li>
                                    </ul>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="checklist categories">
                                    ...
                                    <ul>
                                        @foreach ($categories as $category)
                                            <li>
                                                <input class="form-check-input" type="checkbox" name="category[]" value="{{ $category->name }}" id="flexCheckDefault" {{ request()->get("category") ? (in_array($category->name, request()->get("category")) ? "checked" : "") : "" }}>
                            ...
                            <div class="col-md-12 mt-5 p-5">
                                <button class="btn btn-outline-secondary" type="submit" style="background-color:#0ea2bd; color:white">Apply Filters</button>
                            </div>
                    </div>
                <div class="col-md-9" id="events-data">
                    <div class="row gy-5">
                        <div class="col-md-12">
                                <div class="row height d-flex justify-content-center align-items-center">
                                    <div class="col-md-12">
                                    <div class="form">
                                        <input type="text" name="q" value="{{ request()->get("q") }}" class="form-control form-input" placeholder="Search Events...">
                                    </div>
