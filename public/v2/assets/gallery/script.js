(function($) {
    ...
    var done = false;
    var timer, timeSpent = [], player, players = [];
        ...
        window.onYouTubeIframeAPIReady = function() {
            ...
            $('.video-slide').each(function(i) {
                ...
                if ($(this).attr('data-video-url').indexOf('vimeo.com') !== -1) {
                    ...
                    player = new Vimeo.Player('mbr-video-' + i, options);

                    player.playVideo = Vimeo.play;
                } else {
                    player = new YT.Player('mbr-video-' + i, {
                        height: '100%',
                        width: '100%',
    ...
    function onPlayerStateChange(event) {
        let dataNo  = event.data;
        let videoId = event.target.b.b.videoId;
        let elmId   = $(event.target.a).attr('data-video-num');
        player  = players[elmId];
        if(event.data === 1) {
            if(!timeSpent.length){
        ...
        if(typeof woopra !== 'undefined'){
            console.log('woopra yt play');
            woopra.track("youtube_play", {
                page: document.title,
                event: onYtPlayChange(dataNo),
                video_id: videoId
            });
        }
    }

    function showPercentage(dataNo, videoId){
        var percent = 0;
        ...
        if(percent === 25 || percent === 50 || percent === 75 || percent === 95){
            if(typeof woopra !== 'undefined'){
                console.log('woopra yt play percentage watch ' + percent);
                woopra.track("youtube_play", {
                    page: document.title,
                    event: onYtPlayChange(dataNo),
                    video_id: videoId,
                    percentage_watch:percent
                });
            }
        }
    }

    function onYtPlayChange(dataNo){
        if(dataNo == -1){
            return 'unstarted';
        }
        if(dataNo == 0){
